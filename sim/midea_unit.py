...
|---|-------------------------------------|-------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------|---------------------------|
| 1 | Magnitude of ordinary stepping      | "Each +-1C *change* of reported error steps speed by +-1 increment"          | Each interval's report change contributes at most +-1 to comp_speed regardless of magnitude (sign of the delta only), not a magnitude-proportional step -- PLUS a report exactly at setpoint (`reported_error == 0`) holds (no ordinary step), regardless of delta. Verified against the recorded fixtures: the crafted step-up sequence (`[+1, +2, 0]` offsets, 3 transitions) nets exactly +1 with this rule, matching `guesstimated_comp_speed`'s observed +1-per-event growth in cold_start.json/on_counter_clamp.json -- unchanged by the at-setpoint-holds addition. The crafted step-down sequence (`[-1, -2, +1, 0]` offsets, 4 transitions -- one longer, ending at reported_error 0) originally netted -2 instead of the true -1 under sign-of-delta alone; the spurious extra decrement came solely from the sequence's `stable -> 0` transition, so the at-setpoint-holds rule brings it to -1, matching `guesstimated_comp_speed`'s dead-reckoning (task 013). Previously this one-increment overshoot was dismissed as inside the +-2 tracking tolerance, but in closed-loop replay it compounds across chained step-down sequences (a long taper), doubling the emulated descent rate versus the 2026-06-22 recorded taper -- the at-setpoint-is-special reading is consistent with rule 9's shutdown timer keying on the same condition, so it is not sequence-specific pattern matching. | n/a                       |
        ...
        if self.purging:
            ...
            if not self.purging:
                ...
                if self.ramp_up_flag:
                    ...
                    # Task 013: a report exactly at setpoint holds (no
                    # ordinary step) -- the unit already treats
                    # reported_error == 0 specially for its shutdown timer
                    # (rule 9), so this is consistent thermostat logic, not
                    # sequence pattern-matching. _prev_reported_error still
                    # updates every cycle below regardless, so the
                    # transition OUT of an at-setpoint report still
                    # contributes its delta on the next (non-zero) cycle,
                    # unless that cycle is itself at setpoint.
                    if reported_error != 0:
